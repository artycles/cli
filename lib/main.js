...
Exec.prototype = {
	...
	user : function( domain, options ){
		var artycles = this.api.auth({key: this.creds.key ,secret: this.creds.secret, app: this.creds.app });
	...
	sub : function( domain, options ){
		// fallbacks
		options = options || {};
		var artycles = this.api.auth({keyid: this.creds.key ,secret: this.creds.secret, app: this.creds.app });

		if( domain == "*" ){
			// read the domain list instead
			return artycles.list(function( error, result ) {
		...
		artycles.select( query, function( error, result ) {
			if( error ) return console.log( error );
			output( result );
		});
