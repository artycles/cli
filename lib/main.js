...
var Exec = function( program ){
	...
	// init lib
	this.api = new Artycles( this.config );
...
Exec.prototype = {
	...
	user: function( id, options ){
		...
		if( options.files ) params.type = "files";
	...
	file: function( id, options ){
		...
		// read the product collection for the current user: /files
		params.name = ( options.list ) ? "files" : "file";
